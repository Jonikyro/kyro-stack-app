...
const spriterConfig = { // https://github.com/svg-sprite/svg-sprite#configuration-basics
...
function addSvgToSpriter(files, spriter) {
    ...
    for (const file of files) {
        ...
        consola.info(`Adding icon ${colorize('greenBright', parsedPath.base)} to the sprite`);
...
function writeSpriteToDisk(error, result) {
    ...
    consola.success(`Generated svg-sprite: ${colorize('greenBright', sprite.basename)}`);
}

function generateReactIconComponent() {
    consola.start(`'Generating ${colorize('greenBright', '`<Icon />`')} react component...'`);
...
;`;
    ...
    fs.writeFileSync(
        path.join(normalizeFolder(ICON_COMPONENT_OUTPUT_FOLDER), ICON_COMPONENT_FILENAME),
        template.trim()
    );

    consola.success(`Generated ${colorize('greenBright', ICON_COMPONENT_FILENAME)}`);
}
